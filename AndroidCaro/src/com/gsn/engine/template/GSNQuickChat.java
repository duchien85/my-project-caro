...
public class GSNQuickChat extends GsnStage {
	...
	Label contentLabel;
	Image backGround;
	String arrayString[] = {"Nhanh lên bạn", "Gà thế", "Giỏi thế", "Pro quá", "Không được đâu sói ạ", "Ngu vÃi chưởng", "Lâu thế", "Khó của Nam Cường"};
	ImageButton send;
	public GSNQuickChat(PlayStage parent, float width, float height) {
		...
		send.setClickListener(new ClickListener() {
			...
			public void click(Actor actor, float x, float y) {
				...
				GSNQuickChat.this.parent.chatMe(contentLabel.getText());
				DataProvider.getInstance().caroActivity.sendPacket(PacketFactory.createChat(contentLabel.getText()));
			}
		});
		this.addActor(send);
